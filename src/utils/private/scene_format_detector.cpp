...
#include "../public/scene_format_detector.h"
...
SceneFormatDetectResult DetectSceneFormat(const std::string& path, size_t sniffBytes) {
    ...
    // Tungsten check (JSON-based heuristics, not by extension)
    if (!prefix.empty() && prefix.front() == '{') {
        if (ContainsAny(prefix, {"\"bsdfs\"", "\"integrator\"", "\"renderer\"", "TungstenRenderState.dat", ".wo3"})) {
            r.format = SceneFormat::Tungsten;
            r.reason = "JSON 且含 Tungsten 典型键/特征";
            return r;
        }
    }
