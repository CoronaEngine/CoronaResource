...
void resourceManagerExample() {
    ...
    std::cout << "找到模型文件: " << foundModelPath << "\n";
    
    // 创建 ResourceId
    Corona::ResourceId modelId = Corona::ResourceId::from("model", foundModelPath);
    ...
    // 转换为 Model 类型
    auto model = std::dynamic_pointer_cast<Corona::Model>(resource);
